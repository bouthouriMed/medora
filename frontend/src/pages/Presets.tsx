...
export default function Presets() {
  ...
  const getTypeStyle = (type: PresetType) => {
    return PRESET_TYPES.find((t) => t.value === type)?.color || 'bg-gray-100 text-gray-700 dark:text-gray-300';
  };
      ...
      {isLoading ? (
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
          {filteredPresets.map((preset: Preset) => (
            <div
              key={preset.id}
              className="bg-white dark:bg-gray-800 rounded-2xl shadow-md border border-gray-100 dark:border-gray-700 p-5 hover-lift transition-all"
            >
