...
import { useGetPortalDataQuery, usePortalChatMutation, useGetPortalMessagesQuery, useSendPortalMessageMutation, useCreatePortalPrescriptionRequestMutation } from '../api';
...
function PortalMessageSection({ token }: { token: string }) {
  const [showForm, setShowForm] = useState(false);
  const [doctorId, setDoctorId] = useState('');
  const [subject, setSubject] = useState('');
  const [body, setBody] = useState('');
  const [sent, setSent] = useState(false);
  const { data: messages = [] } = useGetPortalMessagesQuery(token);
  const [sendMessage] = useSendPortalMessageMutation();

  const handleSend = async (e: React.FormEvent) => {
    e.preventDefault();
    await sendMessage({ token, doctorId, subject, body });
    setSent(true);
    setShowForm(false);
    setSubject('');
    setBody('');
  ...
  return (
    <div className="bg-white dark:bg-gray-800 rounded-2xl shadow-lg border border-gray-100 dark:border-gray-700 overflow-hidden">
      <div className="bg-gradient-to-r from-blue-500 to-indigo-600 px-6 py-4 flex items-center justify-between">
        ...
        <button onClick={() => setShowForm(!showForm)} className="px-3 py-1 bg-white/20 text-white rounded-lg text-sm hover:bg-white/30 transition-colors">
      ...
      <div className="p-6">
        {sent && (
          <div className="bg-green-50 border border-green-200 rounded-xl p-3 mb-4 text-sm text-green-700">
            Message sent successfully! Your doctor will respond soon.
          </div>
        )}
        {showForm ? (
          <form onSubmit={handleSend} className="space-y-3">
            ...
            <textarea value={body} onChange={(e) => setBody(e.target.value)} placeholder="Write your message..." required rows={4}
              className="w-full px-3 py-2 border border-gray-300 dark:border-gray-600 rounded-lg bg-white dark:bg-gray-700 text-gray-900 dark:text-white text-sm" />
            <button type="submit" className="w-full px-4 py-2.5 bg-blue-600 text-white rounded-xl hover:bg-blue-700 font-medium text-sm">Send Message</button>
          </form>
        ) : (messages as any[]).length > 0 ? (
          <div className="space-y-3 max-h-64 overflow-y-auto">
            {(messages as any[]).slice(-5).map((msg: any) => (
              <div key={msg.id} className={`p-3 rounded-xl text-sm ${msg.senderType === 'PATIENT' ? 'bg-blue-50 dark:bg-blue-900/20 ml-4' : 'bg-gray-50 dark:bg-gray-700/50 mr-4'}`}>
                <div className="flex justify-between mb-1">
                  <span className="text-xs font-medium text-gray-500">{msg.senderType === 'PATIENT' ? 'You' : 'Doctor'}</span>
                  <span className="text-xs text-gray-400">{new Date(msg.createdAt).toLocaleDateString()}</span>
                </div>
                {msg.subject && <p className="font-medium text-gray-900 dark:text-white text-xs mb-1">{msg.subject}</p>}
                <p className="text-gray-700 dark:text-gray-300">{msg.body}</p>
              </div>
            ))}
          </div>
        ) : (
          <p className="text-gray-500 dark:text-gray-400 text-center py-4 text-sm">No messages yet. Send a message to your doctor.</p>
        )}
      </div>
    </div>
  );
...
function PortalPrescriptionRequest({ token, prescriptions }: { token: string; prescriptions: any[] }) {
  const [showForm, setShowForm] = useState(false);
  const [medication, setMedication] = useState('');
  const [reason, setReason] = useState('');
  const [sent, setSent] = useState(false);
  const [createRequest] = useCreatePortalPrescriptionRequestMutation();

  const handleSubmit = async (e: React.FormEvent) => {
    e.preventDefault();
    await createRequest({ token, medication, reason });
    setSent(true);
    setShowForm(false);
    setMedication('');
    setReason('');
  ...
  return (
    <div className="bg-white dark:bg-gray-800 rounded-2xl shadow-lg border border-gray-100 dark:border-gray-700 overflow-hidden">
      ...
      <div className="p-6">
        {sent && (
          <div className="bg-green-50 border border-green-200 rounded-xl p-3 mb-4 text-sm text-green-700">
            Request submitted! Your doctor will review it shortly.
          </div>
        )}
        {showForm ? (
          <form onSubmit={handleSubmit} className="space-y-3">
            <div>
              <label className="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1">Medication</label>
              <select value={medication} onChange={(e) => setMedication(e.target.value)} required
                className="w-full px-3 py-2 border border-gray-300 dark:border-gray-600 rounded-lg bg-white dark:bg-gray-700 text-gray-900 dark:text-white text-sm">
                <option value="">Select a medication or type new...</option>
                {prescriptions.map((p: any) => <option key={p.id} value={p.medication}>{p.medication} ({p.dosage})</option>)}
              </select>
              {!prescriptions.some((p: any) => p.medication === medication) && (
                <input type="text" value={medication} onChange={(e) => setMedication(e.target.value)} placeholder="Or type medication name"
            ...
            <textarea value={reason} onChange={(e) => setReason(e.target.value)} placeholder="Why do you need this prescription?" rows={3}
              className="w-full px-3 py-2 border border-gray-300 dark:border-gray-600 rounded-lg bg-white dark:bg-gray-700 text-gray-900 dark:text-white text-sm" />
            <button type="submit" className="w-full px-4 py-2.5 bg-teal-600 text-white rounded-xl hover:bg-teal-700 font-medium text-sm">Submit Request</button>
          </form>
        ) : (
          <div className="text-center py-4">
            <p className="text-gray-500 dark:text-gray-400 text-sm">Need a prescription refill or new medication?</p>
            <p className="text-gray-400 text-xs mt-1">Your doctor will review and approve your request.</p>
          </div>
        )}
      </div>
    </div>
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-blue-50 via-white to-purple-50">
      ...
      <main className="max-w-4xl mx-auto px-4 py-8">
        ...
        {/* Message Doctor & Request Prescription */}
        <div className="grid md:grid-cols-2 gap-8 mb-8">
          <PortalMessageSection token={token!} />
          <PortalPrescriptionRequest token={token!} prescriptions={activePrescriptions} />
        </div>
