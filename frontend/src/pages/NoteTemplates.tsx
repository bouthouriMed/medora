  ...
  return (
    <div className="space-y-4 sm:space-y-6">
      ...
      <div className="flex gap-2">
        <button
          onClick={() => setFilter('')}
          className={`px-4 py-2 rounded-xl font-medium transition-colors ${!filter ? 'bg-blue-600 text-white' : 'bg-gray-100 hover:bg-gray-200 text-gray-700'}`}
        >
          All
        </button>
        <button
          onClick={() => setFilter('APPOINTMENT')}
          className={`px-4 py-2 rounded-xl font-medium transition-colors ${filter === 'APPOINTMENT' ? 'bg-blue-600 text-white' : 'bg-gray-100 hover:bg-gray-200 text-gray-700'}`}
        >
          Appointment
        </button>
        <button
          onClick={() => setFilter('INVOICE')}
          className={`px-4 py-2 rounded-xl font-medium transition-colors ${filter === 'INVOICE' ? 'bg-blue-600 text-white' : 'bg-gray-100 hover:bg-gray-200 text-gray-700'}`}
        >
          Invoice
        </button>
      </div>
      ...
      {isLoading ? (
        ...
        <div className="bg-white rounded-2xl shadow-lg border border-gray-100 p-12 text-center">
          ...
          <p className="text-gray-500 mb-6">Create templates for quick note entry</p>
        ...
        <div className="grid gap-4 md:grid-cols-2 lg:grid-cols-3">
          {filteredTemplates.map((template: NoteTemplate) => (
            <div key={template.id} className="bg-white rounded-xl shadow-md border border-gray-100 p-4 hover-lift">
